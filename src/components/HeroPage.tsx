...
export default function HeroPage() {
  ...
  return (
    <section ref={sectionRef} className="page relative bg-bg">
      ...
      <div className="relative z-10 flex flex-col items-center px-6 text-center">
        ...
        {/* animated name */}
        <div className="mb-6">
          <ShreyaTitle size="hero" delayStart={0.6} staggerDelay={0.1} active={isVisible} />
        </div>
        ...
        >
          ...
          >
            <div className="flex flex-col items-center gap-0.5">
              <motion.div
                animate={isVisible ? { opacity: [0.3, 0.8, 0.3] } : { opacity: 0.3 }}
                transition={{ duration: 2.5, repeat: isVisible ? Infinity : 0, ease: 'easeInOut' }}
                className="w-3.5 h-3.5 border-r-[1.5px] border-b-[1.5px] border-white/30 rotate-45"
              />
              <motion.div
                animate={isVisible ? { opacity: [0.15, 0.5, 0.15] } : { opacity: 0.15 }}
