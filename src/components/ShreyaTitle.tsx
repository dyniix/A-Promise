...
export default function ShreyaTitle({
  ...
  useEffect(() => {
    ...
    const drift = async () => {
      ...
      await driftControls.start({ x: 1.5, transition: { duration: 2.5, ease: 'easeInOut' } })
      if (!mountedRef.current) return
      await driftControls.start({ x: 0, transition: { duration: 3, ease: 'easeInOut' } })
      if (!mountedRef.current) return
      timer = setTimeout(drift, 15000)
    }

    timer = setTimeout(drift, 10000)
    ...
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-visible">
        <div className="absolute left-[-10%] w-[40%] h-[180%]">
          <div
            className={`w-full h-full rounded-full blur-[80px] animate-bloom-breathe ${
              glow === 'subtle' ? 'bg-pink/[0.04]' : 'bg-pink/10'
            }`}
          />
        </div>
        <div className="absolute left-[30%] w-[40%] h-[160%]">
          <div
            className={`w-full h-full rounded-full blur-[80px] animate-bloom-breathe ${
              glow === 'subtle' ? 'bg-white/[0.015]' : 'bg-white/[0.03]'
            }`}
        ...
        <div className="absolute right-[-10%] w-[40%] h-[180%]">
          <div
            className={`w-full h-full rounded-full blur-[80px] animate-bloom-breathe ${
              glow === 'subtle' ? 'bg-sky/[0.04]' : 'bg-sky/10'
            }`}
